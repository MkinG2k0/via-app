...
const invertTestKeyState = (s: TestKeyState) => (s === TestKeyState.KeyDown ? TestKeyState.KeyUp : TestKeyState.KeyDown)
		...
		const startTicking = async (
			...
			if (startTest && api && selectedDefinition) {
				...
				try {
					...
					const keysChanges = newFlat.some((val, byteIdx) => val ^ (prevFlat[byteIdx] || 0))
					if (!keysChanges) {
							...
							(res, val, byteIdx) => {
								...
								return Array(Math.max(0, Math.min(8, cols - colOffset)))
									...
									.reduce((resres, _, idx) => {
										...
										resres[matrixIdx] =
											((xor >> idx) & 1) === 1 ? invertTestKeyState(resres[matrixIdx]) : resres[matrixIdx]
										return resres
									}, res)
							},
