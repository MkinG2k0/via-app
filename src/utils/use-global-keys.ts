...
export const useGlobalKeys = (enableGlobalKeys: boolean) => {
	...
	function downHandler(evt: KeyboardEvent) {
		evt.preventDefault()
		if (!startMatrixTest && !evt.repeat && selectedKeys[getIndexByEvent(evt) ?? -1] !== TestKeyState.KeyDown) {
	...
	const upHandler = (evt: KeyboardEvent) => {
		evt.preventDefault()
		if (!startMatrixTest && selectedKeys[getIndexByEvent(evt)] !== TestKeyState.KeyUp) {
			setSelectedKeys((selectedKeys) => ({
				...selectedKeys,
