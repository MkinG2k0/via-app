...
export function isFulfilledPromise<A>(a: PromiseSettledResult<A>): a is PromiseFulfilledResult<A> {
	return a.status === 'fulfilled'
}

export function isAuthorizedDeviceConnected(device: AuthorizedDevice, definitions?: KeyboardDictionary) {
	return (
		definitions &&
