...
let heldKeys = {} as any
let lastEvtTime = 0
export const useKeycodeRecorder = (enableRecording: boolean, recordDelays: boolean) => {
	const keycodeSequenceState = useState<RawKeycodeSequence>([])
	const [, setKeycodeSequence] = keycodeSequenceState
	const keycodes = useMemo(() => getKeycodes().flatMap((menu) => menu.keycodes), [])
			...
			if (enableRecording && !evt.repeat) {
				setKeycodeSequence((keycodeSequence) => {
					...
					if (keycodeSequence.length && recordDelays) {
						keycodeSequence.push([RawKeycodeSequenceAction.Delay, currTime - lastEvtTime])
					}
					if (keycodeLabel) {
