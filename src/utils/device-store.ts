...
export async function syncStore(): Promise<DefinitionIndex> {
	...
	try {
		...
		const v2vpidMap = json.vendorProductIds.v2.reduce((acc: VendorProductIdMap, id) => {
			acc[id] = acc[id] || {}
			acc[id].v2 = acc[id].v3 = true
			return acc
		}, {})

		const vpidMap = json.vendorProductIds.v3.reduce((acc: VendorProductIdMap, def) => {
			acc[def] = acc[def] || {}
			acc[def].v3 = true
			return acc
		}, v2vpidMap)
...
export const getMissingDefinition = async <K extends keyof DefinitionVersionMap>(
...
export const getDefinitionsFromStore = (): KeyboardDictionary => deviceStore.get('definitions')

export const getThemeFromStore = (): ThemeDefinition =>
	THEMES[getThemeNameFromStore() as keyof typeof THEMES] || deviceStore.get('definitionIndex')?.theme
