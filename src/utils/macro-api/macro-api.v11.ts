...
export function validateMacroExpressionV11(expression: string): ValidationResult {
	...
	while ((groups = keycodeBlockRegex.exec(expression))) {
		...
		if (/^\d+$/.test(csv)) {
			...
			const invalidKeycodes = csv
				.replace(/^[-+]/, '')
				.split(',')
				.filter((keycode) => keycode.trim().length && !isAutocompleteKeycode(keycode))
			...
			switch (byte) {
				...
				case KeyActionPrefix:
					...
					switch (byte) {
						case KeyAction.Tap:
							byte = bytes[++i]
							currentSequence.push([RawKeycodeSequenceAction.Tap, (this.byteToKey as any)[byte]])
							break
						case KeyAction.Down:
							byte = bytes[++i]
							currentSequence.push([RawKeycodeSequenceAction.Down, (this.byteToKey as any)[byte]])
							break
						case KeyAction.Up:
							byte = bytes[++i]
							currentSequence.push([RawKeycodeSequenceAction.Up, (this.byteToKey as any)[byte]])
							break
						case KeyAction.Delay:
							...
							currentSequence.push([RawKeycodeSequenceAction.Delay, parseInt(delayValue)])
							break
						default:
				...
				default: {
					...
					if (
						...
						currentSequence[currentSequence.length - 1][0] === RawKeycodeSequenceAction.CharacterStream
					) {
						...
						currentSequence.push([RawKeycodeSequenceAction.CharacterStream, char])
					}
					break
				}
			}
