...
export function optimizedSequenceToRawSequence(sequence: OptimizedKeycodeSequence): RawKeycodeSequence {
	return sequence.flatMap((element) => {
		if (element[0] == GroupedKeycodeSequenceAction.Chord) {
			const makeTurnToKeyAction = (action: RawKeycodeSequenceAction) => (keycode: string) =>
				[action, keycode] as RawKeycodeSequenceItem
			return [...element[1]]
				.map(makeTurnToKeyAction(RawKeycodeSequenceAction.Down))
				.concat([...element[1]].reverse().map(makeTurnToKeyAction(RawKeycodeSequenceAction.Up)))
		} else {
			return [element]
		}
	})
...
export function rawSequenceToOptimizedSequence(sequence: RawKeycodeSequence): OptimizedKeycodeSequence {
	let result: OptimizedKeycodeSequence = []
	result = convertToTapsAndChords(sequence)
	return result
}

function convertToTapsAndChords(sequence: OptimizedKeycodeSequence): OptimizedKeycodeSequence {
	...
	const seq: OptimizedKeycodeSequence = sequence.reduce((p, n) => {
		if (n[0] === RawKeycodeSequenceAction.Tap) {
			return [...p, [RawKeycodeSequenceAction.Down, n[1]], [RawKeycodeSequenceAction.Up, n[1]]]
		}
		return [...p, n]
	}, [] as OptimizedKeycodeSequenceItem[])

	const seq2: OptimizedKeycodeSequence = []
	seq.forEach((element, index) => {
		...
		if (element[0] === RawKeycodeSequenceAction.Down) {
			...
			if (keyDownKeycodes.length > 0 && keyUpKeycode === keyDownKeycodes[unmatchedKeyDownCount - 1]) {
	...
	const seq3: OptimizedKeycodeSequence = []
...
export function convertCharacterTaps(sequence: RawKeycodeSequence): RawKeycodeSequence {
	const result: RawKeycodeSequence = sequence.reduce((p, n) => {
		if (n[0] == RawKeycodeSequenceAction.Down && n[1] in mapKeycodeToCharacterStream) {
			return [...p, [RawKeycodeSequenceAction.Tap, n[1]]]
		} else if (n[0] == RawKeycodeSequenceAction.Up && n[1] in mapKeycodeToCharacterStream) {
			return p
		} else {
			return [...p, n]
		}
...
export function trimLastWait(sequence: RawKeycodeSequence): RawKeycodeSequence {
	if (sequence[sequence.length - 1] && sequence[sequence.length - 1][0] === RawKeycodeSequenceAction.Delay) {
		return sequence.slice(0, -1)
	}
	return sequence
}

export function mergeConsecutiveWaits(sequence: RawKeycodeSequence): RawKeycodeSequence {
	return sequence.reduce((p, n) => {
		if (
			...
			p.splice(-1, 1, [RawKeycodeSequenceAction.Delay, Number(p[p.length - 1][1]) + Number(n[1])])
		} else {
			p.push(n)
		}
		return p
...
export function foldKeydownKeyupKeys(sequence: RawKeycodeSequence): RawKeycodeSequence {
	return sequence.reduce((p, n) => {
		if (
			p[p.length - 1] &&
...
export function convertToCharacterStreams(sequence: RawKeycodeSequence): RawKeycodeSequence {
	// Convert "{KC_A}{KC_B}{KC_C}" to "abc"
	// Convert "{KC_LSFT,KC_A}" to "A"
	const seq: RawKeycodeSequence = sequence.reduce((p, n) => {
		if (n[0] == RawKeycodeSequenceAction.Tap && n[1] in mapKeycodeToCharacterStream) {
			const newChars = mapKeycodeToCharacterStream[n[1]][0]
			if (p[p.length - 1] !== undefined && p[p.length - 1][0] === RawKeycodeSequenceAction.CharacterStream) {
				// append case
				return [
					...p.slice(0, -1),
					[RawKeycodeSequenceAction.CharacterStream, (p[p.length - 1][1] as string) + newChars],
				]
			} else {
	...
	const seq3: RawKeycodeSequence = seq2.reduce((p, n) => {
		if (
			...
			p[p.length - 1][1] = (p[p.length - 1][1] as string).concat(n[1] as string)
			return p
		}
		return [...p, n]
	}, [] as RawKeycodeSequenceItem[])
...
export function sequenceToExpression(sequence: OptimizedKeycodeSequence): string {
	const result: string[] = []
...
export function expressionToSequence(str: string): OptimizedKeycodeSequence {
	const expression: string[] = splitExpression(str)
	const result: OptimizedKeycodeSequence = []
				...
				if (keycodes.length > 0) {
					if (downOrUpAction == null) {
						...
						const action: RawKeycodeSequenceAction =
							downOrUpAction == '+' ? RawKeycodeSequenceAction.Down : RawKeycodeSequenceAction.Up
