...
export const shiftFrom16Bit = (value: number): [number, number] => [value >> 8, value & 255]
...
const shiftBufferFrom16Bit = (buffer: number[]): number[] => buffer.map(shiftFrom16Bit).flatMap((value) => value)
...
export class KeyboardAPI {
	...
	async fastWriteRawMatrix(keymap: number[][]): Promise<void> {
		...
		for (let offset = 0; offset < shiftedData.length; offset += bufferSize) {
			...
			await this.hidCommand(APICommand.DYNAMIC_KEYMAP_SET_BUFFER, [...shiftFrom16Bit(offset), buffer.length, ...buffer])
		}
	}
	...
	async getEncoderValue(layer: number, id: number, isClockwise: boolean): Promise<number> {
		const bytes = [layer, id, Number(isClockwise)]
		const res = await this.hidCommand(APICommand.DYNAMIC_KEYMAP_GET_ENCODER, bytes)
	...
	async getRGBMode() {
		...
		const [, , val] = await this.hidCommand(APICommand.BACKLIGHT_CONFIG_GET_VALUE, bytes)
		return val
	}
	...
	async setColor(colorNumber: number, hue: number, sat: number) {
		const bytes = [colorNumber === 1 ? BACKLIGHT_COLOR_1 : BACKLIGHT_COLOR_2, hue, sat]
		await this.hidCommand(APICommand.BACKLIGHT_CONFIG_SET_VALUE, bytes)
	}
	...
	async setMacroBytes(data: number[]) {
		...
		try {
			...
			await this.hidCommand(APICommand.DYNAMIC_KEYMAP_MACRO_SET_BUFFER, [...shiftFrom16Bit(lastOffset), 1, 0xff])
			...
			await this.hidCommand(APICommand.DYNAMIC_KEYMAP_MACRO_SET_BUFFER, [...lastOffsetBytes, 1, 0x00])
		}
	}
