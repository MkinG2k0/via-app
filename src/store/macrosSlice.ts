...
const macrosSlice = createSlice({
	...
	reducers: {
		...
		saveMacrosSuccess: (state, action: PayloadAction<{ ast: RawKeycodeSequence[] }>) => {
			state.ast = action.payload.ast
		},
		...
		if (protocol < 8) {
			...
			try {
				...
				if (macroApi) {
					...
					dispatch(loadMacrosSuccess({ ast: sequences, macroBufferSize, macroCount }))
				}
			} catch (err) {
				dispatch(setMacrosNotSupported())
			}
...
export const getIsMacroFeatureSupported = (state: RootState) => state.macros.isFeatureSupported

export const getAST = (state: RootState) => state.macros.ast
export const getMacroBufferSize = (state: RootState) => state.macros.macroBufferSize
