...
const definitionsSlice = createSlice({
	...
	reducers: {
		...
		loadInitialCustomDefinitions: (state, action: PayloadAction<KeyboardDictionary>) => {
		...
		loadCustomDefinitions: (
			...
			definitions.forEach((definition) => {
				const definitionEntry = state.customDefinitions[definition.vendorProductId] ?? {}
				if (version === 'v2') {
...
export const getBaseDefinitions = (state: RootState) => state.definitions.definitions
export const getCustomDefinitions = (state: RootState) => state.definitions.customDefinitions
export const getLayoutOptionsMap = (state: RootState) => state.definitions.layoutOptionsMap

export const getDefinitions = createSelector(
	getBaseDefinitions,
	getCustomDefinitions,
	(definitions, customDefinitions) => {
		return Object.entries(customDefinitions).reduce((p, [id, definitionMap]) => {
...
export const getSelectedLayoutOptions = createSelector(
	...
	getSelectedDevicePath,
	(definition, map, path) =>
		(path && map[path]) || (definition && definition.layouts.labels && definition.layouts.labels.map((_) => 0)) || [],
)
	...
	(layoutOptions, definition) =>
		(definition
			? layoutOptions.flatMap(
					(option, idx) => (definition.layouts.optionKeys[idx] && definition.layouts.optionKeys[idx][option]) || [],
				)
			: []) as VIAKey[],
)
...
export const loadStoredCustomDefinitions = (): AppThunk => async (dispatch, getState) => {
	try {
...
export const loadLayoutOptions = (): AppThunk => async (dispatch, getState) => {
	...
	const api = getSelectedKeyboardAPI(state)
	if (!connectedDevice || !selectedDefinition || !selectedDefinition.layouts.labels || !api) {
		return
	}
...
export const reloadDefinitions =
	...
	async (dispatch, getState) => {
		...
		const missingDefinitionsSettledPromises = await Promise.allSettled(
			missingDevicesToFetchDefinitions.map((device) => getMissingDefinition(device, device.requiredDefinitionVersion)),
		)
		...
		const missingDefinitions = missingDefinitionsSettledPromises.filter(isFulfilledPromise).map((res) => res.value)
