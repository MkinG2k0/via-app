...
const menusSlice = createSlice({
	...
	reducers: {
		...
		updateCommonMenus: (state, action: PayloadAction<{ commonMenuMap: CommonMenusMap }>) => {
...
export const { updateShowKeyPainter, updateSelectedCustomMenuData, updateCustomMenuData } = menusSlice.actions
...
export const updateV3MenuData =
	...
	async (dispatch, getState) => {
		...
		if (commands.length !== 0 && protocol >= 11) {
			...
			const maxLedIndex = Math.max(...definition.layouts.keys.map((key) => key.li ?? -1))
...
export const getCommonMenusDataMap = (state: RootState) => state.menus.commonMenusMap

export const getShowKeyPainter = (state: RootState) => state.menus.showKeyPainter

export const getCustomMenuDataMap = (state: RootState) => state.menus.customMenuDataMap
...
export const getV3Menus = createSelector(getSelectedDefinition, (definition) => {
	...
	return (definition.menus || [])
		...
		.map((menu, idx) => (isVIAMenu(menu) ? compileMenu('custom_menu', 3, menu, idx) : menu))
})
...
export const getV3MenuComponents = createSelector(getSelectedDefinition, (definition) => {
	...
	return (definition.menus || [])
		.flatMap(tryResolveCommonMenu)
		.map((menu: any, idx) =>
			isVIAMenu(menu) ? makeCustomMenu(compileMenu('custom_menu', 3, menu, idx), idx) : menu,
