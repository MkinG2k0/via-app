...
export const reloadConnectedDevices = (): AppThunk => async (dispatch, getState) => {
	...
	const recognisedDevices = await getRecognisedDevices(supportedIds, forceRequest)

	const protocolVersions = await Promise.all(
		recognisedDevices.map((device) => new KeyboardAPI(device.path).getProtocolVersion()),
	)

	const recognisedDevicesWithBadProtocol = recognisedDevices.filter((_, i) => protocolVersions[i] === -1)
	...
	const connectedDevices = authorizedDevices
		.filter((device, i) => isAuthorizedDeviceConnected(device, newDefinitions))
	...
	authorizedDevices.filter((device) => !isAuthorizedDeviceConnected(device, newDefinitions)).forEach(tryForgetDevice)
