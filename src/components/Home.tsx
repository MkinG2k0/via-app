...
export const Home: React.FC<HomeProps> = (props) => {
	...
	const toggleLights = async () => {
		...
		if (
			...
			getLightingDefinition(selectedDefinition.lighting).supportedLightingValues.includes(
	...
	return !hasHIDSupport && !OVERRIDE_HID_CHECK ? (
		<ErrorHome ref={homeElem} tabIndex={0}>
			<UsbError>
				...
				<p>
					Looks like there was a problem getting USB detection working. Right now, we only support{' '}
					<UsbErrorWebHIDLink href={'https://caniuse.com/?search=webhid'} target={'_blank'}>
