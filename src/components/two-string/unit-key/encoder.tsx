...
const EncoderKeyContent2 = styled.div<{ $innerPadding: number; $size: number }>`
	...
	background-image: radial-gradient(currentColor var(--half-size), transparent var(--half-size-p1)),
		radial-gradient(rgba(255, 255, 255, 0.6) var(--half-size), transparent var(--half-size-p1)),
		radial-gradient(rgba(0, 0, 0, 0.2) var(--half-size), transparent var(--half-size-p05p)),
		radial-gradient(rgba(0, 0, 0, 0.2) var(--half-size), transparent var(--half-size-p05p));
...
export const EncoderKey = (props: {
	...
	return (
		<EncoderKeyContainer onClick={props.onClick} style={props.style}>
			<EncoderKeyContent2 $innerPadding={(5 * props.size) / 52} $size={props.size && Number(props.size)} />
		</EncoderKeyContainer>
	)
}
