...
function inputIsBasicByte(input: string, basicKeyToByte: Record<string, number>): boolean {
	const keyCode = input.trim().toUpperCase()
	return keyCode in basicKeyToByte
}

function basicByteFromInput(input: string, basicKeyToByte: Record<string, number>): number {
	const keyCode = input.trim().toUpperCase()
	return basicKeyToByte[keyCode]
}

function inputIsAdvancedKeyCode(input: string, basicKeyToByte: Record<string, number>): boolean {
	const keyCode = input.trim().toUpperCase()
	return advancedStringToKeycode(keyCode, basicKeyToByte) !== 0
}

function advancedKeyCodeFromInput(input: string, basicKeyToByte: Record<string, number>): number {
	const keyCode = input.trim().toUpperCase()
...
function inputIsValid(input: string, basicKeyToByte: Record<string, number>): boolean {
...
export const KeycodeModal: React.FC<KeycodeModalProps> = (props) => {
	...
	const defaultInput = anyKeycodeToString(props.defaultValue as number, basicKeyToByte, byteToKey)

	const { getMenuProps, getInputProps, highlightedIndex, inputValue, getItemProps, isOpen } = useCombobox({
		items: inputItems,
	...
	return (
		<ModalBackground>
			<ModalContainer>
				<PromptText>Please enter your desired QMK keycode or hex code:</PromptText>
				<div>
					<div>
						<TextInput {...getInputProps()} placeholder={defaultInput || 'KC_NO, 0xFF, etc.'} type={'text'} />
					</div>
					...
					>
						{isOpen &&
							inputItems.map((item, index) => (
								<AutocompleteItemRow {...getItemProps({ item, index })} key={item.code}>
									<AutocompleteItem entity={item} key={item.code} selected={highlightedIndex === index} />
								</AutocompleteItemRow>
							))}
					</AutocompleteContainer>
				</div>
				<RowDiv>
					...
					<PrimaryAccentButton
						disabled={!isValid}
						onClick={() => {
							props.onConfirm(keycodeFromInput(inputValue as any, basicKeyToByte) as any)
						}}
					>
						Confirm
					</PrimaryAccentButton>
