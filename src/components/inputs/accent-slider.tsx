...
const Slider = styled.span<{ $ischecked?: boolean }>`
	...
	right: 0;
	bottom: 0;
	background-color: ${(props) => (props.$ischecked ? 'var(--color_accent)' : 'var(--bg_control)')};
	...
	&:before {
		...
		bottom: 4px;
		border-radius: 4px;
		background-color: ${(props) => (!props.$ischecked ? 'var(--bg_icon)' : 'var(--color_inside-accent)')};
		-webkit-transition: 0.4s;
		transition: 0.4s;
...
export function AccentSlider(props: Props) {
	...
	return (
		<Switch>
			<HiddenInput checked={isHiddenChecked} onChange={hiddenOnChange} ref={ref} type={'checkbox'} />
