...
const PreviousColorOption = styled.div<{ $selected: boolean }>`
	...
	border-color: ${(props) => (props.$selected ? 'var(--color_accent)' : 'var(--border_color_cell)')};
`

export const ConnectedColorPalettePicker: React.FC = () => {
	const dispatch = useDispatch()
	const setColor = useCallback((hue: number, sat: number) => dispatch(setSelectedPaletteColor([hue, sat])), [dispatch])
	useEffect(() => {
		...
		<ColorPalettePickerContainer>
			<PreviousColorContainer>
				{initialColors.map((savedColor, idx) => {
					const isSelected = selectedColor[0] === savedColor[0] && selectedColor[1] === savedColor[1]
					return (
						<PreviousColorOption
							$selected={isSelected}
							key={idx}
			...
			<ColorPicker
				...
				isSelected={colorPickerColor[0] === selectedColor[0] && colorPickerColor[1] === selectedColor[1]}
				onMouseUp={() => {
