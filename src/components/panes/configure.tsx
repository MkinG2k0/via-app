...
const Rows = [Keycode, Macros, Layouts, Lighting, SaveLoad, RotaryEncoder, ...makeCustomMenus([])]
...
const getRowsForKeyboard = (): typeof Rows => {
	...
	if (!selectedDefinition) {
		...
		return [
			...filterInferredRows(selectedDefinition, showMacros, numberOfLayers, [Keycode, Layouts, Macros, SaveLoad]),
			...v3Menus,
		]
	} else {
		return []
...
const filterInferredRows = (
	...
	const filteredRows = rows.filter((row) => !removeList.includes(row)) as typeof Rows
	return filteredRows
}
...
const Loader: React.FC<{
	...
	return (
		<LoaderPane>
			<ChippyLoader progress={loadProgress || null} theme={theme} />
...
export const ConfigurePane = () => {
	...
	return showLoader ? (
		renderMode === '2D' ? (
			<Loader loadProgress={loadProgress} selectedDefinition={selectedDefinition || null} />
		) : null
	) : (
		<ConfigureBasePane>
			<ConfigureGrid />
		...
		<>
			...
			<Grid style={{ pointerEvents: 'none' }}>
				<MenuCell style={{ pointerEvents: 'all' }}>
					<MenuContainer>
						{(KeyboardRows || []).map(({ Icon, Title }: { Icon: any; Title: string }, idx: number) => (
							<Row $selected={selectedRow === idx} key={idx} onClick={(_) => setRow(idx)}>
								<IconContainer>
									<Icon />
