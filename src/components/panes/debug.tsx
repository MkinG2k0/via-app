...
import { getBaseDefinitions, getBasicKeyToByte, getCustomDefinitions, getDefinitions } from 'src/store/definitionsSlice'
import { getConnectedDevices, getSelectedKeyboardAPI } from 'src/store/devicesSlice'
import { useAppSelector } from 'src/store/hooks'
import { getSelected256PaletteColor, setSelectedPaletteColor } from 'src/store/keymapSlice'
...
import { ControlRow, Detail, IndentedControlRow, Label, OverflowCell, SubLabel } from './grid'
	...
	return (
		<ControlGroup>
			...
			<ControlRow>
				...
				<Detail>
					<PelpiKeycodeInput meta={{}} setValue={setKeycode} value={keycode} />
				</Detail>
			</ControlRow>
			<ControlRow>
				<Label>
				...
				<Detail>
					<ArrayColorPicker color={colorVal} setColor={(hue, sat) => setColorVal([hue, sat])} />
				</Detail>
			</ControlRow>
			<ControlRow>
				<Label>
				...
				<Detail>
					<ColorPalettePicker
						...
						setColor={(hue, sat) => dispatch(setSelectedPaletteColor([hue, sat]))}
					/>
				</Detail>
			</ControlRow>
			<ControlRow>
				...
				<Detail>
					<AccentRange defaultValue={rangeVal} max={100} min={0} onChange={setRangeVal} />
				</Detail>
			</ControlRow>
			<ControlRow>
				<Label>{Number(isChecked)}</Label>
				<Detail>
					<AccentSlider isChecked={isChecked} onChange={setIsChecked} />
				</Detail>
			</ControlRow>
			<ControlRow>
				<Label>{Number(selectionVal)}</Label>
				<Detail>
					<AccentSelect
						defaultValue={selectOptions[selectionVal]}
						onChange={(option: any) => {
							option && setSelectionVal(Number(option.value))
						}}
			...
			<MacroRecorder
				...
				undoMacro={() => null}
			/>
			<MacroRecorder isDelaySupported={true} saveMacro={() => null} setUnsavedMacro={(_) => _} undoMacro={() => null} />
		</ControlGroup>
	)
}
				...
				<Container>
					...
					<ControlGroup>
						...
						<ControlRow>
							...
							<Detail>
								<AccentSlider isChecked={showMatrix} onChange={(val) => setShowMatrix(val)} />
							</Detail>
						</ControlRow>
						<ControlRow>
								...
								<AccentButton
									onClick={() => {
										...
										const selectedOptionKeys = optionKeys
											? Object.entries(optionKeys).flatMap(([key, options]) => options[0])
											: []
										const displayedKeys = [...keys, ...selectedOptionKeys]
										if (selectedKey !== undefined) {
											setSelectedKey(getNextKey(selectedKey, displayedKeys) || 0)
										}
									}}
								>
									Next
							...
							<ControlRow>
								...
								<Detail>
									<AccentSelect
										...
										onChange={(option: any) => option && setSelectedDefinition(Number(option.value))}
										options={options}
									/>
								</Detail>
							</ControlRow>
					...
					{api && (
						<ControlGroup>
							<ControlGroupHeader>Connected Device Debugging</ControlGroupHeader>
							<ControlRow>
								<Label>EEPROM Reset</Label>
								<Detail>
									<AccentButton onClick={() => api.resetEEPROM()}>Reset</AccentButton>
								</Detail>
							</ControlRow>
							<ControlRow>
								<Label>Bootloader Jump</Label>
								<Detail>
									<AccentButton onClick={() => api.jumpToBootloader()}>Jump</AccentButton>
								</Detail>
							</ControlRow>
							<ControlRow>
								<Label>Clear all macros</Label>
								<Detail>
									<AccentButton onClick={() => api.resetMacros()}>Clear</AccentButton>
								</Detail>
							</ControlRow>
							<ControlRow>
								...
								<Detail>
									<AccentButton
										onClick={async () => {
											...
											await Array(1000)
												.fill(0)
												.map((_) => api.getKeyboardValue(KeyboardValue.SWITCH_MATRIX_STATE, [], 20))
					...
					<ControlGroup>
							...
							if (definitionEntry) {
								return (
									<IndentedControlRow key={device.path}>
										<SubLabel>{(definitionEntry[1] as VIADefinitionV2 | VIADefinitionV3).name}</SubLabel>
										<Detail>0x{device.vendorProductId.toString(16).toUpperCase()}</Detail>
									</IndentedControlRow>
								)
							}
							return null
						...
						<ControlRow>
							...
							<Detail>{Object.values(localDefinitions).length} Definitions</Detail>
						</ControlRow>
						...
						<ControlRow>
							<details>
								<summary>
									...
									<Detail>{Object.values(remoteDefinitions).length} Definitions</Detail>
								</summary>
								{Object.values(remoteDefinitions).map(([id, definition], idx) => (
									<IndentedControlRow key={idx}>
										<SubLabel>{definition.name}</SubLabel>
										<Detail>
											0x
