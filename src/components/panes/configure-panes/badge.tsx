...
const KeyboardButton = styled.button<{ $selected?: boolean }>`
	...
	width: 100%;
	border: none;
	background: ${(props) => (props.$selected ? 'var(--bg_icon-highlighted)' : 'transparent')};
	color: ${(props) => (props.$selected ? 'var(--color_icon_highlighted)' : 'var(--color_label-highlighted)')};
	cursor: pointer;
	text-align: left;
	font-size: 14px;
	text-transform: uppercase;
	...
	&:hover {
		border: none;
		background: ${(props) => (props.$selected ? 'var(--bg_icon-highlighted)' : 'var(--bg_control)')};
		color: ${(props) => (props.$selected ? 'var(--color_control-highlighted)' : 'var(--color_label-highlighted)')};
	}
`
