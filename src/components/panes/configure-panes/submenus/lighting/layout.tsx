...
const BooleanControls: [LightingValue, string][] = [
	...
	[LightingValue.BACKLIGHT_DISABLE_HHKB_BLOCKER_LEDS, 'Disable HHKB Blocker LEDs'],
]
...
export const Pane: FC = () => {
	...
	if (lightingDefinition.supportedLightingValues.length !== 0) {
		const controls = BooleanControls.filter(
			(control) => lightingDefinition.supportedLightingValues.indexOf(control[0]) !== -1,
		)
						...
						<ControlRow key={command}>
							...
							<Detail>
								<AccentSlider
									...
									onChange={(val) => dispatch(updateBacklightValue(command, Number(val)))}
								/>
							</Detail>
						</ControlRow>
					)
