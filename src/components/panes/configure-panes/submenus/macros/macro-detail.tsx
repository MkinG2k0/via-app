...
const MacroTab = styled.span<{ $selected: boolean }>`
	...
	color: ${(props) => (props.$selected ? 'var(--color_accent)' : 'var(--bg_icon)')};
	cursor: pointer;
	&:hover {
		color: ${(props) => (props.$selected ? 'var(--color_accent)' : 'var(--bg_icon-highlighted)')};
	}
`
...
const printBytesUsed = (bytesUsed: number, bufferSize: number) => {
	...
	return `${convertedBytesUsed.toFixed(scale)} / ${convertedBufferSize.toFixed(scale)} ${suffix} space used`
}
...
const BufferSizeUsage = () => {
	...
	return (
		<ProgressBarContainer>
			...
			<ProgressBarTooltip>{printBytesUsed(bytesUsed, bufferSize)}</ProgressBarTooltip>
		</ProgressBarContainer>
	)
}
...
export const MacroDetailPane: React.FC<Props> = (props) => {
	...
	return (
		<>
			<CenterTabContainer>
				<TabBar>
					<MacroTab $selected={!showAdvancedView} onClick={() => setShowAdvancedView(false)}>
						<FontAwesomeIcon icon={faClapperboard} />
					</MacroTab>
					<MacroTab $selected={showAdvancedView} onClick={() => setShowAdvancedView(true)}>
