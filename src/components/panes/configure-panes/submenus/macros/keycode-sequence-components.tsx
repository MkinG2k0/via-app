...
import { OptimizedKeycodeSequenceItem, RawKeycodeSequenceAction } from 'src/utils/macro-api/types'
...
export const WaitInput: React.FC<{
	...
	const onChange = (evt: ChangeEvent<HTMLInputElement>) => {
		if (Number(evt.target.value) > 0 && Number(evt.target.value) < 100000) {
			// Update external value
			props.updateValue(props.index, Number(evt.target.value))
		}
	}

	return (
		<KeycodeSequenceWait>
			<NumberInput onBeforeInput={onBeforeInput as any} onChange={onChange} ref={inputRef} value={props.value} />
			ms
		</KeycodeSequenceWait>
	)
}

export const getSequenceItemComponent = (action: OptimizedKeycodeSequenceItem[0]) =>
