...
export const Pane: FC = () => {
					...
					.map((_, i) =>
						Promise.all(
							Array(numberOfLayers)
								.fill(0)
								.map((_, j) =>
									Promise.all([api.getEncoderValue(j, i, false), api.getEncoderValue(j, i, true)]).then(
										(a) =>
											a.map((keyByte) => getCodeForByte(keyByte, basicKeyToByte, byteToKey) || '') as [string, string],
									),
								),
						),
					),
	...
	const saveLayout = async () => {
		...
		const suggestedName = `${name.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase()}.layout.json`
		try {
			...
			const saveFile: ViaSaveFile = {
				...
				layers: rawLayers.map(
					(layer: { keymap: number[] }) =>
						layer.keymap.map((keyByte: number) => getCodeForByte(keyByte, basicKeyToByte, byteToKey) || ''), // TODO: should empty string be empty keycode instead?
				),
	...
	const loadLayout = ([file]: Blob[]) => {
		...
		reader.onload = async () => {
			...
			const keymap: number[][] = saveFile.layers.map((layer) =>
				layer.map((key) => getByteForCode(`${deprecatedKeycodes[key] ?? key}`, basicKeyToByte)),
			)
							...
							encoder.map((layer, layerId) =>
								Promise.all([
									api.setEncoderValue(
										...
										id,
										false,
										getByteForCode(`${deprecatedKeycodes[layer[0]] ?? layer[0]}`, basicKeyToByte),
									),
									api.setEncoderValue(
										layerId,
										id,
										true,
										getByteForCode(`${deprecatedKeycodes[layer[1]] ?? layer[1]}`, basicKeyToByte),
									),
								]),
							),
						),
