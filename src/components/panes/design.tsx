...
export const DesignTab: FC = () => {
	...
	return (
		...
		>
			...
			>
				This feature is intended for development purposes. If your keyboard is not recognized automatically by VIA,
				please contact your keyboard's manufacturer or vendor.
			</MessageDialog>
				...
				<SpanOverflowCell>
					<Container>
						<ControlRow>
							...
							<Detail>
								<AccentUploadButton
									...
									multiple
									onLoad={(files) => {
										importDefinitions(Array.from(files), definitionVersion, dispatch, setErrors)
									}}
								>
									Load
								</AccentUploadButton>
						...
						<ControlRow>
							...
							<Detail>
								<AccentSlider
									...
									onChange={(val) => dispatch(updateDesignDefinitionVersion(val ? 'v2' : 'v3'))}
								/>
							</Detail>
						</ControlRow>
						{definition && (
									...
									<Detail>
										<AccentSelect
											onChange={(option: any) => {
												...
												if (option) {
													dispatch(updateSelectedDefinitionIndex(Number(option.value)))
												}
											}}
											options={options}
						...
						<ControlRow>
							...
							<Detail>{Object.values(versionDefinitions).length} Definitions</Detail>
						</ControlRow>
						{versionDefinitions.map((definition) => {
							return (
								<IndentedControlRow key={`${definitionVersion}-${definition[definitionVersion].vendorProductId}`}>
									<SubLabel>{definition[definitionVersion].name}</SubLabel>
									<Detail>
										{formatNumberAsHex(definition[definitionVersion].vendorProductId, 8)}
