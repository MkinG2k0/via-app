...
import { faCancel, faComputer, faDownload, faKeyboard, faWarning } from '@fortawesome/free-solid-svg-icons'
	...
	return (
		<>
			<IconButtonGroupContainer style={{ margin: '10px 15px' }}>
				<IconButtonContainer disabled={!hasErrors} onClick={clear}>
					<FontAwesomeIcon color={'var(--color_label)'} icon={faCancel} size={'sm'} />
					<IconButtonTooltip>Clear</IconButtonTooltip>
				</IconButtonContainer>
				<IconButtonContainer disabled={!hasErrors} onClick={save}>
					<FontAwesomeIcon color={'var(--color_label)'} icon={faDownload} size={'sm'} />
...
const AppErrors: React.FC<{}> = ({}) => {
	...
	return (
		...
		>
			{errors.map(({ timestamp, deviceInfo: { productId, productName, vendorId }, message: error }) => (
...
const ErrorPanes: [ErrorPaneMenu, React.FC, IconProp, string][] = [[ErrorPaneMenu.App, AppErrors, faComputer, 'App']]
		...
		return (
			<Link to={'/errors'}>
				<CategoryIconContainer $selected={isSelectedRoute}>
					<FontAwesomeIcon color={isSelectedRoute ? 'inherit' : 'gold'} icon={ErrorsPaneConfig.icon} size={'xl'} />
					<CategoryMenuTooltip>
