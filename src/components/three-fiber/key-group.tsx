...
import { calculateKeyboardFrameDimensions, CSSVarObject, KeycapMetric } from 'src/utils/keyboard-rendering'
...
import { getKeycapSharedProps, getKeysKeys, getLabels } from '../n-links/key-group'
...
const getSRGBArray = (keyColors: number[][]) => {
	return keyColors.map(([hue, sat]) => {
		...
		const srgbStr = `#${new Color(rgbStr).convertSRGBToLinear().getHexString()}`
...
export const KeyGroup: React.FC<KeyGroupProps<ThreeEvent<MouseEvent>>> = (props) => {
	...
	const keyColorPalette = props.keyColors ? getSRGBArray(props.keyColors) : selectedSRGBTheme
	...
	}, [keys, keyColorPalette, props.onKeycapPointerDown, props.onKeycapPointerOver])
	...
	const elems = useMemo(() => {
		return props.keys.map((k, i) => {
			...
			return k.d ? null : (
				<Keycap
					keycapGeometry={
						((keycapScene.getObjectByName(meshKey) as any) || keycapScene.getObjectByName('K-R1-100')).geometry
					}
					{...getKeycapSharedProps(k, i, props, keysKeys, selectedKeyIndex, labels, skipFontCheck)}
				/>
			)
		})
	}, [
