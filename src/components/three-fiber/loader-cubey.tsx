...
export const LoaderCubey: React.FC<{ theme: Theme; visible: boolean }> = React.memo(({ visible, theme }) => {
	...
	useFrame(({ clock }) => {
		if (visible) {
			spinnerRef.current.rotation.z = Math.sin(clock.elapsedTime) * (Math.PI / 40)
			spinnerRef.current.rotation.y = Math.PI + Math.sin(0.6 * clock.elapsedTime) * (Math.PI / 16)
			spinnerRef.current.position.y = yInit + 0.2 * Math.sin(clock.elapsedTime)
		}
	})

	return (
		<>
			<group position={[0, yInit, -19]} scale={0.6}>
