...
const paintEncoder = (
	...
	const [canvasWidth, canvasHeight] = [canvasSize * widthMultiplier, canvasSize * heightMultiplier]
...
const paintDebugLines = (canvas: HTMLCanvasElement, keycapRect: Rect, faceRect: Rect) => {
	const context = canvas.getContext('2d')
	if (context == null) {
		return
	}
	...
	context.lineWidth = 2
	context.beginPath()
	context.moveTo(keycapRect.bl.x * canvas.width, (1 - keycapRect.bl.y) * canvas.height)
	context.lineTo(keycapRect.bl.x * canvas.width, (1 - keycapRect.tr.y) * canvas.height)
	context.lineTo(keycapRect.tr.x * canvas.width, (1 - keycapRect.tr.y) * canvas.height)
	context.lineTo(keycapRect.tr.x * canvas.width, (1 - keycapRect.bl.y) * canvas.height)
	context.lineTo(keycapRect.bl.x * canvas.width, (1 - keycapRect.bl.y) * canvas.height)
	context.stroke()
	context.beginPath()
	context.moveTo(faceRect.bl.x * canvas.width, (1 - faceRect.bl.y) * canvas.height)
	context.lineTo(faceRect.bl.x * canvas.width, (1 - faceRect.tr.y) * canvas.height)
	context.lineTo(faceRect.tr.x * canvas.width, (1 - faceRect.tr.y) * canvas.height)
	context.lineTo(faceRect.tr.x * canvas.width, (1 - faceRect.bl.y) * canvas.height)
	context.lineTo(faceRect.bl.x * canvas.width, (1 - faceRect.bl.y) * canvas.height)
	context.stroke()
}

const paintKeycapLabel = (canvas: HTMLCanvasElement, rect: Rect, legendColor: string, label: any) => {
	const context = canvas.getContext('2d')
	if (context == null) {
		return
	}
	...
	if (label === undefined) {
		...
		const fontSize = 52
		const fontHeightTU = (0.75 * fontSize) / canvas.height
		const topLabelOffset = label.offset[0] * fontHeightTU
		const bottomLabelOffset = label.offset[1] * fontHeightTU
		...
		const fontSize = 37.5 * label.size
		const fontHeightTU = (0.75 * fontSize) / canvas.height
		const faceMidLeftY = (rect.tr.y + rect.bl.y) / 2
		context.font = `bold ${fontSize}px ${fontFamily}`
		context.fillText(
			label.label,
			(rect.bl.x + centerLabelMargin.x) * canvas.width,
			(1 - (faceMidLeftY - 0.5 * fontHeightTU - centerLabelMargin.y)) * canvas.height,
		)
		// return if label would have overflowed so that we know to show tooltip
		return context.measureText(label.centerLabel).width > (rect.tr.x - (rect.bl.x + centerLabelMargin.x)) * canvas.width
...
export const Keycap: React.FC<ThreeFiberKeycapProps> = React.memo((props) => {
	...
	const redraw = React.useCallback(() => {
		if (canvasRef.current && color) {
			if (shouldRotate) {
				...
				setOverflowsTexture(Boolean(doesOverflow))
			}
	...
	const maxZ = keycapGeometry.boundingBox!.max.z
	...
	const [keycapZ, rotationZ] =
		pressedState === KeycapState.Pressed ? [zDown, rotation[2]] : [zUp, rotation[2] + Math.PI * Number(shouldRotate)]
	...
	const [meshOnClick, meshOnPointerOver, meshOnPointerOut, meshOnPointerDown] = useMemo(() => {
		const noop = () => {}
		return disabled
			? [noop, noop, noop, noop]
		...
		<>
			...
			>
				<AniMeshMaterial attach={'material'} color={selected ? glow.y : b}>
					<canvasTexture attach={'map'} image={canvasRef.current} ref={textureRef as any} />
				</AniMeshMaterial>
			</animated.mesh>
			{(macroData || overflowsTexture) && (
				<React.Suspense fallback={null}>
					<animated.group position={props.position} position-z={20} scale={tooltipScale}>
						<Html
							style={{
								pointerEvents: 'none',
							}}
							transform
						>
							<KeycapTooltip>{macroData || (label && label.tooltipLabel)}</KeycapTooltip>
						</Html>
					</animated.group>
				</React.Suspense>
			)}
