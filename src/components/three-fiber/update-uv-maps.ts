	...
	useEffect(() => {
		...
		Object.values(keycapScene.children).forEach((mesh) => {
			...
			if (!u100.geometry.attributes.uv) {
				u100.geometry.setAttribute('uv', new BufferAttribute(new Float32Array(pos100u.count * 2), 2, false))
			}
			...
			for (let i = 0; i < u100.geometry.attributes.uv.count; i++) {
				...
				newUv[2 * i + 1] = (size1u * (pos100u.array[i * 3 + 1] - min.y + offsetToCorner)) / unitScale
			}
