		...
		<>
			...
			>
				<Canvas flat={true} shadows style={{ overflow: 'visible' }}>
					<Lights />
					<KeyboardBG color={accentColor} onClick={terrainOnClick} visible={!hideTerrainBG} />
					<OrbitControls enabled={false} />
					<Camera />
					<LoaderCubey theme={theme} visible={hideTerrainBG && !selectedDefinition} />
					<Html center position={[0, hideTerrainBG ? (!selectedDefinition ? -1 : 0) : 10, -19]}>
						{showAuthorizeButton ? (
							!selectedDefinition ? (
								<AccentButtonLarge onClick={() => dispatch(reloadConnectedDevices())} style={{ width: 'max-content' }}>
									Authorize device
									<FontAwesomeIcon icon={faUnlock} style={{ marginLeft: '10px' }} />
								</AccentButtonLarge>
							) : (
								<>
									<div
	...
	return renderAllLights ? (
		<>
			...
			<SpotLight
				...
				target={targetObj}
			></SpotLight>
			<pointLight intensity={0.8} position={[x, y, z]} />
			<pointLight intensity={0.8} position={[-x, y, z]} />
		</>
	) : (
		<>
			<ambientLight intensity={0.4} />
			<pointLight intensity={1.5} position={[-0.5, y, z]} />
		</>
	)
}, shallowEqual)
...
const getRouteX = (route: string) => {
	...
	const testPosition = Number(-spaceMultiplier)
	...
	return (
		<>
			<group visible={loadProgress === 1}>
				<ConfigureKeyboard dimensions={dimensions} nDimension={'3D'} selectable={configureKeyboardIsSelectable} />
			</group>
