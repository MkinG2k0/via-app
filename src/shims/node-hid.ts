import type { AuthorizedDevice, ConnectedDevice, WebVIADevice } from '../types/types'
// This is a bit cray
const globalBuffer: {
...
const filterHIDDevices = (devices: HIDDevice[]) =>
	devices.filter((device) =>
		device.collections?.some((collection) => collection.usage === 0x61 && collection.usagePage === 0xff60),
	)

const getVIAPathIdentifier = () =>
	(self.crypto && self.crypto.randomUUID && self.crypto.randomUUID()) || `via-path:${Math.random()}`
